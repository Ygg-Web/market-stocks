    ...
    <template>
      <v-row>
        ...
        >
          <v-card>
            <v-card-title 
                class="subheading font-weight-bold flex-column"
            >{{ stock.name }}</v-card-title>
            <v-divider></v-divider>
            <v-list dense>
              <v-list-item
                  v-for="(field, index) in filteredFieldNames"
                  :key="index"
              >
                <v-list-item-content>{{ fields(field) }}</v-list-item-content>
                <v-list-item-content class="align-end justify-center">{{ stock[field] }}</v-list-item-content>
              </v-list-item>
            </v-list>
            <v-divider></v-divider>
            <v-card-actions class="pa-5">
              ...
              >
                ...
                ></DialogStocks>
                <DialogGrafic
                    :historyPrice="stock.historyPrice"
                    :historyTime="stock.historyTime"
                ></DialogGrafic>
              </v-layout>
            </v-card-actions>
          </v-card>
